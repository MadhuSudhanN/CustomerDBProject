...
<%@page import="com.Service.FeatchCust"%>
...
<body style="margin-left: 100px; margin-right: 100px">
	...
	<table border=10px style="padding-right: 200px; padding-left: 200px">
		...
		List<CustomerEntity> list = CustomerDAOImp.search(request.getParameter("search"),request.getParameter("name"));
		out.print(request.getParameter("name")+" "+request.getParameter("search"));
		for (CustomerEntity x : list) {
		%>
		<tr>

			<form action="/process" method="post">
				<td><input class="id" type="number" name="cid"
					value="<%=x.getCid()%>" placeholder="<%=x.getCid()%>"></td>
				...
				<td><input class="id1x" type="text" name="phone"
					placeholder="<%=x.getPhone()%>"></td>
